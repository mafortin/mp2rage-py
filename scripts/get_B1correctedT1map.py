...
from func.functions import coreg_and_resample_B1map, resample_to_match
...
#b1_resamp = coreg_and_resample_B1map(b1map_pathname, mp2rage_pathname)

# Resample the moving image to match the fixed image
b1resamp_data, b1_resamp_img = resample_to_match(b1map_pathname, mp2rage_pathname)
...
# Show B1 map
fig = plt.figure(figsize=(12, 6))
plotting.plot_anat(b1_resamp_img, figure=fig, cut_coords=(0,0,0), display_mode='ortho', draw_cross=False, vmin=None, vmax=None, colorbar=True, annotate=True, title="B1 map coregistered and resampled")
plt.show()
